...
static mut STACK: Stack<SIZE> = Stack::new();

/// Exclusive address of the stack top.
#[used]
#[no_mangle]
static STACK_TOP: Safe<*const u8> = Safe::new(unsafe { STACK.top() });
...
fn write_canary() {
    unsafe {
        core::ptr::write(canary_address(), CANARY);
    }
}
...
pub fn current_canary() -> u64 {
    unsafe { core::ptr::read(canary_address()) }
}
